...
```
fonts.js              the 30 built-ins, identity, the css2 URL builder
font-catalog.js       lazy lookup over the bundled Google list
google-catalog.data.js  GENERATED by scripts/fetch-google-fonts.mjs
font-manager.js       download, store, register; the network policy
font-previews.js      batched specimens for families you do not have
font-table.js         which fonts a document uses; the .pkd manifest
```

**One id per family.** `layer.text.font` holds a built-in id (`playfair`) or
`google:<Exact Family Name>`. The name *is* the id — not a slug — because that
string is already the key for css2, for CSS `font-family`, for `FontFace` and
for the catalogue, and because it must resolve with nothing else loaded:
reopening a `.pkd` offline knows which family the layer wants from the id alone.
`normalizeFontId()` reconciles the dialects that have historically been written
there (display names, raw CSS families, `google:` ids for built-ins) and is
applied **at the leaves** — `fontStack`, `getFontFamily`, `postScriptFace`, and
the resolve point in `text-render.js` — so no caller can be forgotten. A
built-in beats a download: `google:Roboto` collapses to `roboto` and keeps its
PostScript faces.

**A css2 request must match the family.** Asking for a weight range a family
does not offer is a hard failure, not a fallback. `css2Url()` emits one of three
shapes — no axis for a single static weight, every weight listed for a static
family, `wght@min..max` only where there is a real variable axis. Getting this
wrong is not hypothetical: the hardcoded `100..900` that preceded it meant ten
of the fourteen bundled Google families never loaded, silently, for the whole
life of the feature.

**`unicodeRange` on `FontFace` is required, not an optimisation.** Omit it and
the browser may pick the Cyrillic subset to draw Latin text, which renders as
notdef boxes rather than as an error. For the same reason readiness is checked
with a sample string: `document.fonts.check(css, 'Ag')`.

**Fonts are exempt from `prune()`.** Evicting an autosave costs seconds;
evicting a font silently changes how every document using it looks. A ceiling
that refuses new downloads, plus explicit removal, instead.

**Format modules do not import the manager.** `font-table.js` takes a capability
provider that `font-manager.js` registers on load, so `pkd.js` and
`psd-write.js` can record real weights without dragging IndexedDB, the
preferences store and the catalogue into a save.
